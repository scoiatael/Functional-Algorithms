...
structure InfDepthTree : INFDEPTHTREE =
struct
  open SuspStream
  datatype 'a tree = Fork of 'a * 'a treeList | Leaf
  withtype 'a treeList = 'a tree stream

  fun constTree a =
  let
    val this = ref Leaf
    val _ = this := Fork(a, constStream (!this))
  in !this
  end

  fun tMap f Leaf = Leaf
    | tMap f (Fork(a, st)) = Fork(f a, sMap (fn t => tMap f t) st)

  fun subTree [] a = a
    | subTree _ Leaf = Leaf
    | subTree (x::xs) (Fork(_, t)) = subTree xs (nth x t)

  fun unfold f arg =
  let val (head, st) = f arg
  in Fork(head, sMap (fn s => unfold f s) st)
  end
end
